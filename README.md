...
> 💡 Replace `docs/screenshots/demo.gif` with a screen recording of the app in action — a GIF under ~15 MB embeds directly and autoplays on GitHub without any extra setup. 🎞️
...
The Electron desktop application 💻 communicates with a Flask backend 🌶️ over a REST API 📡, where a trained XGBoost 🌲 classifier evaluates incoming URLs and returns a prediction with a calibrated confidence score 📊.
...
```
flowchart LR
    A[👤 User] --> B[⚛️ Electron Desktop App]
...
flowchart TD
    ...
    Model --> Suspicious[⚠️ Verdict: Suspicious]
    Model --> Malicious[⛔ Verdict: Malicious]
    Safe --> Render[🖥️ Load page normally]
...
| Technology | Purpose |
|---|---|
| ⚛️ Electron | Desktop application shell |
...
| 🏗️ Domain structure | Subdomain depth, hyphen count, entropy | Randomly generated or padded domains score differently than real brand domains |
...
The model is an XGBoost 🌲 classifier, tuned via Optuna 🎯 hyperparameter search and calibrated with sigmoid scaling so its output can be read as a genuine probability rather than a raw decision score. Class imbalance in training data is corrected using SMOTE ⚖️ oversampling.
...
│
├── backend/
│   ├── app.py              # 🌶️ Flask API: /, /health, /predict
│   ├── train.py            # 🏋️ Dataset generation + model training
...
│   ├── index.html
│   ├── index.css
│   ├── main.js              # ⚛️ Electron main process + trust score engine
│   ├── preload.js           # 🌉 IPC bridge
│   ├── renderer.js          # 🖼️ UI logic, tabs, shield badge
│   └── config-store.js
│
...
Download the latest Windows installer directly: ⬇️
...
> ⚠️ Trust Flow is not yet code-signed, so Windows SmartScreen 🛡️ may show a warning on first run — this is expected for free, unsigned software. Click **More info → Run anyway** to proceed. ✅
...
**2. Install desktop dependencies** ⚛️
...
No — `/predict` requires reaching the Railway-hosted backend ☁️. If the backend is unreachable, the app fails safe to a neutral default score rather than blocking all browsing.
...
Yes — add it in the in-app Settings panel ⚙️ to enable VirusTotal cross-referencing.

**🧠 How is the ML model trained?**
See `backend/train.py` 🏋️ — a labeled dataset of safe and phishing URLs (including synthetic homograph/punycode variants) is used to train and calibrate an XGBoost classifier, validated against a held-out unseen-URL test set. 🌲
...
**Danyal Fayaz** 🧠 Project Lead · 🤖 ML Engineer · 💻 Full-Stack Developer
...
This project is licensed under the MIT License ⚖️ — see [LICENSE](LICENSE) for details.
...
💼 LinkedIn: [linkedin.com/in/danyal-fayaz-b84820373](https://linkedin.com/in/danyal-fayaz-b84820373)
